...
it('should throw an error for too large members set', () => {
  expect(() => maybeEnum(new Array(0x81).fill(0))).to.throw(Error)
})
...
it('should encode arbitrary string correctly', () => {
  ...
  const encoded = c.encode(maybeEnum(members), 'hello world')
...
it('should decode arbitrary string correctly', () => {
  ...
  const decoded = c.decode(maybeEnum(members), bytes)

  expect(decoded).to.eql('hello world')
})

it('should throw an error for arbitrary strings longer than 0x80 charcters', () => {
  expect(() => c.encode(maybeEnum([]), 'h'.repeat(0x80))).to.not.throw(Error)
  expect(() => c.encode(maybeEnum([]), 'h'.repeat(0x81))).to.throw(Error)
})
